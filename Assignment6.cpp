...
// Inputs: make run in terminal will get puzzles 1-5 to execute
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {                   // loop through files
        ...
        if (solutions.empty()) {                       // if solution is empty
            cout << "No solution found." << endl;      // print no soltiion found
        } else {
            cout << "Solutions found: " << solutions.size() << endl;   // print total valid soltions
            for (size_t s = 0; s < solutions.size(); s++) {      // loop thru each solition on list
                cout << "Solution #" << (s + 1) << ":" << endl;    // print the silitons
